...
export default function VerifyPage() {
  ...
  const [statusMessage, setStatusMessage] = useState('웹캠 인증을 위한 감지를 시작합니다.')
  ...
  useEffect(() => {
    ...
    const callYoloApi = async () => {
      ...
      try {
        ...
        const detectedSuccess = data.success === true && detectTarget.includes(detectedClass)

        if (detectedSuccess) {
          localSuccessCount += 1
          setSuccessCount(localSuccessCount)
          updateStatus(`감지됨: ${detectedClass} (${localSuccessCount}/3)`)
        } else {
          localSuccessCount = 0
          setSuccessCount(0)
  ...
  return (
    <PageLayout>
      <div className="verify-page">
        <h1 className="title">미션 인증을 위한 웹캠 활성화</h1>
        ...
        <div className="verify-body">
          ...
          {isServerWaiting && (
            <div className="verify-warning">객체 탐지 서버 연결 대기 중</div>
          )}
        </div>
      </div>
    </PageLayout>
