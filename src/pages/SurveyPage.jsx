...
export default function SurveyPage() {
  ...
  const isMedicationTimeStep = currentKey === 'medicationTime'

  const submitSurvey = async (finalAnswers) => {
    localStorage.setItem('initialSurvey', JSON.stringify(finalAnswers))
    ...
    try {
      ...
      await authFetch('/api/v1/users/me/status', {
        method: 'PUT',
        body: JSON.stringify(buildRequestBody(finalAnswers)),
      })
  ...
  const moveNext = async (nextAnswers) => {
  ...
  const handleSelect = async (option) => {
    ...
    if (isMedicationTimeStep) {
      setAnswers((prev) => {
        const selectedTimes = prev.medicationTime || []

        const nextTimes = selectedTimes.includes(option.value)
          ? selectedTimes.filter((time) => time !== option.value)
          : [...selectedTimes, option.value]

        return {
          ...prev,
          medicationTime: nextTimes,
        }
      })
  ...
  const handleMedicationNext = async () => {
    ...
    if (answers.medicationTime.length === 0) {
      alert('복용 시간을 하나 이상 선택해주세요.')
      return
    }
        ...
        <div className="survey-card">
          ...
          <div className="survey-options">
            {question.options.map((opt) => {
              const isSelected = isMedicationTimeStep
                ? answers.medicationTime.includes(opt.value)
                : answers[currentKey] === opt.value

              return (
                <div key={opt.value} className="survey-option-item">
                  <Button
